...
（参加者の方はコードを提出するだけで大丈夫です。当日ビデオ会議でつないだりすることはありません。）
...
反対に小さい数を大きい数の右に書くと、右と左を加算することを意味します。  
...
- GitHub CopilotのようなAIコード支援機能はオフにしてください（答えそのもののコードが自動生成されかねないため）
- ロジックは自力で考えてください（ネット上に公開されている変換アルゴリズムを見ないようにしてください）
- 他の人のプルリクエストを見ないようにしてください（提出し終わってこれ以上絶対に変更しない、と誓ったら見てもOKです）
- コードで大喜利をしないでください（1から3999まで全部ベタ書きしました！とか、`test/pattern.yml`の内容をそのまま変換ルールとして利用しました！とか）
